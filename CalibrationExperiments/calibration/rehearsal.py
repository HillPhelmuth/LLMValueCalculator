...
from calibration.model_mapping import ArtificialAnalysisMapping, ArtificialAnalysisSnapshot
...
def run_rehearsal(output: str | Path) -> Path:
    ...
    aa_snapshot = ArtificialAnalysisSnapshot.from_mappings(
        (mapping,), snapshot_date="2026-07-18", source_citations=("recorded-fixture",), catalog=catalog
    )
    ...
    with SqliteRunStore(database) as store:
        ...
        try:
            asyncio.run(interrupted.run(code_commit="rehearsal"))
        except RuntimeError as error:
            if "intentional rehearsal interruption" not in str(error):
                raise
        else:
            raise RehearsalError("Interruption injection did not stop the first pass")
        run_id = store.latest_run_id()
        if store.run_summary(run_id)["status"] != "failed":
            raise RehearsalError("Interrupted pass did not persist a failed resumable run")
        ...
        summary = asyncio.run(resumed.run(resume_run_id=run_id, code_commit="rehearsal"))
        ...
        if len(attempts) != 20 or len({row["request_hash"] for row in attempts}) != 20:
            raise RehearsalError("Resume produced duplicate or missing request attempts")
        ...
        export = export_run_to_parquet(store, run_id, root / "exports", artifacts=artifacts)
        ...
        candidate = _candidate_from_fit(fit, manifest, fitting_path, root / "candidate-profile.json")
        baseline = baseline_profile()
        scenarios = build_scenario_matrix(("general",), ("normal",), ("none",), ("low",), ("one",), ("normal",))

        def evaluate(scenario, model_id: str, profile_hash: str) -> RecommendationSnapshot:
        ...
        diff_path = write_regression_report(diffs, root / "reports" / "scenario-diff.json")
        metrics = collect_run_metrics(store, run_id, expected_cells=20).to_json()
        exact_scores = [row for row in store.rows_for_export("scores", run_id) if row["scorer_name"] == "answer_exact_match"]
        success_rate = sum(bool(row["success"]) for row in exact_scores) / max(1, len(exact_scores))
        run_provenance = store.run_summary(run_id).get("provenance")
        if not isinstance(run_provenance, dict) or not run_provenance.get("provenance_id"):
        ...
        card = CalibrationCard(
            ...
            coverage={"expected_cells": 20, "observed_cells": len(attempts), "fitting_rows": len(fitting.rows)},
            exclusions=fitting.quality.exclusions,
            costs={"estimated_usd": metrics["estimated_cost_usd"], "actual_usd": metrics["actual_cost_usd"]},
            fit_diagnostics=fit.diagnostics.to_json(),
            holdout_metrics={"success_rate": success_rate, "split_values": {"fit": success_rate, "model_holdout": success_rate}},
            intervals=fit.to_json()["intervals"],
            sensitivity={"resume_duplicate_request_hashes": 0, "transport_events": len(transport_events)},
            decisions=({"parameter": "promotion", "decision": "review", "rationale": "candidate remains unpromoted until explicit review"},),
        ...
        evidence = {
            ...
            "material_recommendation_fraction": sum(item.material for item in diffs) / len(diffs),
            "intervals": fit.to_json()["intervals"],
        ...
        if promotion_gate.passed:
            raise RehearsalError("Rehearsal candidate unexpectedly passed without explicit review")
        (root / "promotion-check.json").write_text(
            json.dumps(promotion_gate.to_json(), sort_keys=True, indent=2), encoding="utf-8"
        )
        ...
        if artifact_errors or provenance_errors or gate["status"] != "completed":
            raise RehearsalError(f"Rehearsal gates failed: {artifact_errors}, {provenance_errors}, {gate}")
...
def _candidate_from_fit(fit, manifest, fitting_path: Path, output: Path):
    ...
    profile = CalibrationProfile(
        ...
        curve_segments=tuple({"upper": 10.0 * (index + 1) if index < 5 else None, "slope": value} for index, value in enumerate(slopes)),
    ...
    output.write_text(json.dumps(profile.to_json(), sort_keys=True, indent=2), encoding="utf-8")
    return profile
