...
def _evidence(candidate_hash: str, baseline_hash: str) -> PromotionEvidence:
    return PromotionEvidence(
        ...
        held_out_brier_improvement=0.02,
...
def test_promotion_checks_and_append_only_rollback() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        assert [event["event"] for event in store.history()["history"]] == [
            "promote",
            "rollback",
        ]
...
def test_promotion_rejects_duplication_or_missing_review() -> None:
    ...
    bad = replace(
        evidence,
        duplicate_pathways=1,
        reviewers=(),
        held_out_brier_improvement=0.005,
    )
    ...
    assert not check.checks["held_out_brier_improvement"]
    with tempfile.TemporaryDirectory() as directory:
        with unittest.TestCase().assertRaises(PromotionError):
            PromotionStore(Path(directory) / "profiles").promote(
                candidate, baseline, bad
            )
